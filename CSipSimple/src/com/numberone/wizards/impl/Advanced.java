package com.numberone.wizards.impl;
...
*/
	...
	public SipProfile buildAccount(SipProfile account) {
		...
		String sip=AccountWizard.sip.getText().toString()
		...
		Log.d(THIS_FILE,pass+","+user+"@"+sip+proxy);
		account.display_name = "MobileSoftphone";//accountDisplayName.getText().trim();
		String[] serverParts = sip.split(":");//accountServer.getText().split(":");
		account.acc_id = user.trim() + 
			" <sip:" + SipUri.encodeUser(user.trim()) + "@" + serverParts[0].trim() + ">";
		
		account.reg_uri = "sip:" + sip;//accountServer.getText();
	//	account.username ="+"+user;
		account.realm = "*";
		if(user.startsWith("+"))
	        account.username = user.substring(1)+"@"+sip;//getText(accountAuthId).trim();
		else
        account.username = user+"@"+sip;//getText(accountAuthId).trim();
    	Log.d("Account fetched",account.username+","+proxy);
 /*       if (TextUtils.isEmpty(account.username)) {
            account.username = getText(accountUserName).trim();
        }
*/		account.data = pass;//getText(accountPassword);
