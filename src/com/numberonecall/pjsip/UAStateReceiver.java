...
*/	
				...
				if(account != null && account.try_clean_registers != 0 && account.active) {
					...
					if(pState != null) {
						...
						if(pState.getStatusCode() > 200
							...
							int state =0;// pjsua.acc_clean_all_registrations(accountId);
	...
	public void initService(PjSipService srv) {
		...
		if (eventLock == null) {
			...
			eventLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.numberonecall.inEventLock");
			eventLock.setReferenceCounted(true);

		}
		if(ongoingCallLock == null) {
            PowerManager pman = (PowerManager) pjService.service.getSystemService(Context.POWER_SERVICE);
            ongoingCallLock = pman.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.numberonecall.ongoingCallLock");
